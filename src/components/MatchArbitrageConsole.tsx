    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-amber-500/30 bg-gradient-to-br from-slate-900 via-slate-900 to-amber-950/30 p-6 backdrop-blur-md">
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-amber-500/20 px-2.5 py-0.5 font-mono text-xs font-bold text-amber-400 border border-amber-500/30">
            PROTOCOL: nx-sourcing-contract v0.1.0
          </span>
          <span className="font-mono text-xs text-slate-400">|</span>
          <span className="font-mono text-xs text-slate-300">Dyadic Matchmaking & Arbitrage Engine</span>
        </div>
        <h2 className="mt-2 text-2xl font-bold text-white">
          WTB Demand vs. Sourced Supply Arbitrage Workbench
        </h2>
        <p className="mt-1 text-sm text-slate-300 max-w-3xl">
          Pair active Want-to-Buy buyer demands from NormsExchange with autonomous web-sourced equipment listings. Calculate compatibility scores, margin spreads, warranty and inspection SLAs, and synthesize verified outbox contract envelopes for Codex.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border border-emerald-500/40 bg-slate-900/80 p-5 backdrop-blur-md">
            ...
            >
              {wtbListings.map((wtb) => (
                <option key={wtb.id} value={wtb.id}>
                  [{wtb.id}] {wtb.make} {wtb.model} - Max: ${wtb.priceTarget.toLocaleString()} ({wtb.contact?.entityName})
                </option>
              ))}
            </select>
          </div>
          ...
          {currentWtb && (
            <div className="mt-4 space-y-3 rounded-lg border border-slate-800 bg-slate-950/90 p-4 text-xs font-mono">
              ...
              <div className="flex items-baseline justify-between border-b border-slate-800 pb-2">
                <span className="text-slate-400">Target Make / Model:</span>
                <span className="text-emerald-300 font-bold">{currentWtb.make} {currentWtb.model}</span>
              </div>
              <div className="flex items-baseline justify-between border-b border-slate-800 pb-2">
                <span className="text-slate-400">Max Buyer Budget:</span>
                <span className="text-emerald-400 font-bold text-sm">${currentWtb.priceTarget.toLocaleString()} USD</span>
              </div>
              <div className="flex items-baseline justify-between border-b border-slate-800 pb-2">
        ...
        <div className="rounded-xl border border-cyan-500/40 bg-slate-900/80 p-5 backdrop-blur-md">
            ...
            >
              {wtsListings.map((wts) => (
                <option key={wts.id} value={wts.id}>
                  [{wts.id}] {wts.make} {wts.model} - Ask: ${wts.priceTarget.toLocaleString()} ({wts.contact?.entityName})
                </option>
              ))}
            </select>
          </div>
          ...
          {currentWts && (
            <div className="mt-4 space-y-3 rounded-lg border border-slate-800 bg-slate-950/90 p-4 text-xs font-mono">
              ...
              <div className="flex items-baseline justify-between border-b border-slate-800 pb-2">
                <span className="text-slate-400">Sourced Item:</span>
                <span className="text-cyan-300 font-bold">{currentWts.make} {currentWts.model} ({currentWts.year})</span>
              </div>
              <div className="flex items-baseline justify-between border-b border-slate-800 pb-2">
                <span className="text-slate-400">Asking Price:</span>
                <span className="text-cyan-400 font-bold text-sm">${currentWts.priceTarget.toLocaleString()} USD</span>
              </div>
              <div className="flex items-baseline justify-between border-b border-slate-800 pb-2">
