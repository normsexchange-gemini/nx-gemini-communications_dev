...
interface NavbarProps {
  activeTab: "exchange" | "database" | "sources" | "wtb-demands" | "autonomous-feed" | "sourcing-scanner" | "matches" | "git-outbox" | "operating-norms" | "goals";
  setActiveTab: (tab: "exchange" | "database" | "sources" | "wtb-demands" | "autonomous-feed" | "sourcing-scanner" | "matches" | "git-outbox" | "operating-norms" | "goals") => void;
  wtbCount: number;
  wtsCount: number;
  matchCount: number;
  totalDbCount?: number;
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-[11px] text-slate-400">
              CORRIDOR: <span className="text-amber-400 font-semibold">LA / US ⇄ VIETNAM</span>
            </span>
            <span className="hidden text-slate-700 sm:inline">|</span>
            <span className="hidden font-mono text-[11px] text-cyan-400/90 sm:inline">
              DATABASE: <span className="text-slate-300">1,000 Listings Active</span>
            </span>
            <span className="hidden text-slate-700 sm:inline">|</span>
            <span className="hidden font-mono text-[11px] text-purple-400/90 sm:inline">
              INTAKE: <span className="text-slate-300">contract-v0.2.0 pinned</span>
            </span>
          </div>

          <div className="flex items-center gap-3 font-mono text-[11px]">
            <span className="hidden text-slate-400 md:inline">UPSTREAM: <span className="text-white font-medium">normsexchange-dev/nx-gemini-intake_dev</span></span>
            <span className="text-slate-700">|</span>
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
        ...
        <nav className="hidden items-center gap-1 rounded-lg border border-slate-800 bg-slate-900/60 p-1 lg:flex">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-all ${
              activeTab === "sources"
                ? "bg-gradient-to-r from-cyan-600 to-indigo-600 text-white shadow-sm font-bold"
                : "text-slate-400 hover:bg-slate-800/60 hover:text-slate-200"
            }`}
          >
            <Server className="h-3.5 w-3.5 text-cyan-400" />
            <span>Sources</span>
            <span className="ml-0.5 rounded-full bg-indigo-950 px-1.5 py-0.2 text-[9px] font-mono text-cyan-300 border border-cyan-500/30">
              Metered
            </span>
          </button>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium transition-all ${
              activeTab === "goals"
                ? "bg-gradient-to-r from-indigo-600 to-rose-600 text-white shadow-sm font-bold"
                : "text-slate-400 hover:bg-slate-800/60 hover:text-slate-200"
            }`}
          >
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`whitespace-nowrap rounded-md px-3 py-1 text-xs font-semibold ${
              activeTab === "sources" ? "bg-cyan-600 text-white" : "text-cyan-400"
            }`}
          >
            Sources (Metered)
          </button>
          ...
          <button
            onClick={() => setActiveTab("autonomous-feed")}
            className={`whitespace-nowrap rounded-md px-3 py-1 text-xs font-semibold ${
              activeTab === "autonomous-feed" ? "bg-gradient-to-r from-cyan-600 to-indigo-600 text-white" : "text-cyan-400"
            }`}
          >
            Live Scanner Feed
          </button>
          ...
          <button
            ...
            className={`whitespace-nowrap rounded-md px-3 py-1 text-xs font-bold ${
              activeTab === "goals" ? "bg-rose-600 text-white" : "text-slate-400"
            }`}
          >
            /goal Objectives
          </button>
